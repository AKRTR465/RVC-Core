...
__all__ = ["Encoder", "Decoder", "MultiHeadAttention", "FFN"]
...
class Decoder(nn.Module):
    ...
    def forward(self, x, x_mask, h, h_mask):
        ...
        self_attn_mask = self_attn_mask * x_mask.unsqueeze(2)
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(
        ...
        if self.proximal_bias:
            if t_s != t_t:
                raise ValueError("Proximal bias is only available for self-attention.")
        ...
        if mask is not None:
            ...
            if self.block_length is not None:
                if t_s != t_t:
                    raise ValueError("Local attention is only available for self-attention.")
                block_mask = (
