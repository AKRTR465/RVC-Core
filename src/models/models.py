...
class Generator(torch.nn.Module):
    ...
    def forward(
        ...
        for i in range(self.num_upsamples):
            ...
            x = self.ups[i](x)
            x = apply_resblock_stack(
                self.resblocks, self.num_kernels, x, i, "Generator"
            )
        x = F.leaky_relu(x)
        x = self.conv_post(x)
        x = torch.tanh(x)
    ...
    def forward(
        ...
        for i, (ups, noise_convs) in enumerate(zip(self.ups, self.noise_convs)):
            if i < self.num_upsamples:
                ...
                x = x + x_source
                x = apply_resblock_stack(
                    self.resblocks, self.num_kernels, x, i, "GeneratorNSF"
                )
        x = F.leaky_relu(x)
        x = self.conv_post(x)
        x = torch.tanh(x)
...
class MultiPeriodDiscriminator(torch.nn.Module):
    def __init__(self, use_spectral_norm=False, periods=None):
